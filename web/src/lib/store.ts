...
export const fetchSensorData = async () => {
  ...
  try {
    const [voltageResponse, healthResponse] = await Promise.all([
      adaApi.getAnalogVoltage(),
      adaApi.getSensorHealth().catch(() => null) // Health might not be available
    ]);
...
export const fetchSystemStatus = async () => {
  ...
  try {
    const statusResponse = await adaApi.getStatus();
...
export const fetchConfig = async () => {
  ...
  try {
    const configResponse = await adaApi.getConfig();
...
export const fetchAnalytics = async (params?: any) => {
  ...
  try {
    const analyticsResponse = await adaApi.getAnalytics();
